...
local create_keymaps = function()
  -- Normal mode acceleration_motions
  ...
  -- Visual mode acceleration_motions
  for _, motion in ipairs(config_module.items.acceleration_motions_visual) do
    vim.api.nvim_set_keymap(
      'v',
      motion,
      string.format("<CMD>lua require'accelerated-jk'.move_to('%s')<CR>", motion),
  ...
  -- Create mappings for both normal and visual modes
